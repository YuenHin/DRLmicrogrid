...
from Model.Conversion_units import  CTP
...
"创建line"
line1 = Line("line_1", 4000, line_price=-0.001, Single=True, MG_point=True, from_MG="CPP", to_MG="MG", time_num=time)
line2 = Line("line_2", 4000, line_price=-0.001, Single=True, MG_point=True, from_MG="CPP", to_MG="MG", time_num=time)
line3 = Line("line_3", 4000, line_price=-0.001, Single=True, MG_point=True, from_MG="CPP", to_MG="MG", time_num=time)

"创建一个CTP"
ctp = CTP("CTP", 0.05, 0.05, 0.05, time_num=time, line1=line1, line2=line2, line3=line3)
...
node03 = Node("node_CTP", devices=np.array([ctp]), sLine=np.array([line2, line3]), rLine=np.array([line1]), time_num=time, type='CTP')

"创建一个微网，将node01节点放在该微网（可扩展）"
MG01 = MG("MG_CPP", node=np.array([node01]), id=1, type="MG01", time_num=time)

"创建一个微网，将node02节点放在该微网（可扩展）"
MG02 = MG("MG_D_e", node=np.array([node02, node03]), id=1, type="MG02", time_num=time)

"创建一个多微网"
MMGs = MMGs(np.array([MG01,MG02]))

C, intergrality, num = MMGs_logic(MMGs, path, flag=False)
