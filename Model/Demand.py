...
"柔性负荷"
class FL:
    def __init__(self, name, type, p_total, response_power_min, response_power_max,
                 id=1, MG_id=1, ramping_rate=0.25, time_num=24, stochastic_value=10):
        self.name = name
        self.id = id
        self.MG_id = MG_id
        self.time_num = time_num
        ...
        # 响应功率的最大最小值
        self.response_power_min = response_power_min
        self.response_power_max = response_power_max

        self.p = np.zeros(self.time_num)
        self.p_min = np.zeros(self.time_num)
        self.p_max = np.zeros(self.time_num)

        self.p_limit_max = None
        self.p_limit_min = None
    ...
    def __getData(self):
        if self.type == "e":
            # kw
            self.p = getDataFromExcel("Data\Data_demands.xlsx", 1, 25, self.id, self.id + self.research_day) * 365 * (
                        self.p_total / self.research_day)
            self.p = self.p.flatten()
            self.p_min = self.p * (1 - self.ramping_rate)
            self.p_max = self.p * (1 + self.ramping_rate)
        ...
        if self.type == "g":
            self.p = getDataFromExcel("Data\Data_demands.xlsx", 1, 25, self.id + 50,
                                      self.id + 50 + self.research_day) * 365 * (
                             self.p_total / self.research_day)
            self.p = self.p.flatten()
            self.p_min = self.p * (1 - self.ramping_rate)
            self.p_max = self.p * (1 + self.ramping_rate)
        ...
        if self.type == "th":
            self.p = getDataFromExcel("Data\Data_demands.xlsx", 1, 25, self.id + 100,
                                      self.id + 100 + self.research_day) * 365 * (
                             self.p_total / self.research_day)
            self.p = self.p.flatten()
            self.p_min = self.p * (1 - self.ramping_rate)
            self.p_max = self.p * (1 + self.ramping_rate)
        ...
        if self.type == "h":
            self.p = getDataFromExcel("Data\Data_demands.xlsx", 1, 25, self.id + 150,
                                      self.id + 150 + self.research_day) * 365 * (
                             self.p_total / self.research_day)
            self.p = self.p.flatten()
            self.p_min = self.p * (1 - self.ramping_rate)
            self.p_max = self.p * (1 + self.ramping_rate)
    ...
    def constraints(self, num):
        ...
        CreatConstraintsByText(self.time_num, B, -self.response_power_max, self.response_power_max, num)
