...
class S:
    ...
    def __getData(self):
        ...
        each_hour_step = self.time_num / 24 # 每个小时的充放电步数
    ...
    def get_action(self, step, num, action, cur_episode):
        ...
        if cur_episode >= 0:  # 训练的后半段再用智能体给的动作
            action_ = action
            if step == 1:
        ...
        if cur_episode < -1:  # 训练的前半段用MILP求解的值，将最优解放进经验池
        # 我需要控制不确定变化后不会跳出范围
    ...
    def fix(self, num):
        ...
        for i in range(self.time_num):
            # num.bl[self.constraints_num + self.time_num + i ] =  self.x[self.time_num + i]
            # num.bu[self.constraints_num + self.time_num + i] = self.x[self.time_num + i]
            B = np.array([
                [self.name + "E" + str(i + 1), 1],
            ])
            CreatConstraintsByText(1, B, self.x[self.time_num + i], self.x[self.time_num + i], num)
