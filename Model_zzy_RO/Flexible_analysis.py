...
class FA:
    ...
    def __set_C(self):
        ...
        for i in range(6, 13):  # 7点至13点
            ...
            self.c[i] = -0.048
            self.c[i + self.time_num] = -0.048
        for i in range(13, 16):  # 14点至16点
            # self.c[i] = -0.8 * 0.7
            self.c[i] = -0.038
            self.c[i + self.time_num] = -0.038
        ...
        for i in range(6, 13):
            ...
            self.c[i + self.time_num * 2] = -0.04
            self.c[i + self.time_num * 3] = -0.04
        for i in range(13, 16):
            # self.c[i] = -0.7 * 0.8
            self.c[i + self.time_num * 2] = -0.03
            self.c[i + self.time_num * 3] = -0.03
        ...
        for i in range(6, 13):
            ...
            self.c[i + self.time_num * 4] = -0.043
            self.c[i + self.time_num * 5] = -0.043
        for i in range(13, 16):
            # self.c[i] = -0.7 * 0.8
            self.c[i + self.time_num * 4] = -0.033
            self.c[i + self.time_num * 5] = -0.033
        for i in range(16, 22):
            # self.c[i] = -0.7 * 1.1 * 0.7
            self.c[i + self.time_num * 4] = -0.043
            self.c[i + self.time_num * 5] = -0.043
        ...
        for i in range(6, 13):
            # self.c[i] = -0.7 * 1.1 * 0.7
            self.c[i + self.time_num * 6] = -0.041
            self.c[i + self.time_num * 7] = -0.041
        for i in range(13, 16):
            # self.c[i] = -0.7 * 0.8
            self.c[i + self.time_num * 6] = -0.031
            self.c[i + self.time_num * 7] = -0.031
        for i in range(16, 22):
            # self.c[i] = -0.7 * 1.1 * 0.7
            self.c[i + self.time_num * 6] = -0.041
            self.c[i + self.time_num * 7] = -0.041
    ...
    def remember_realValue(self, step, num):
        ...
        B = np.array([
            [self.name + "fm_d_e" + str(step), 1],
        ])
        CreatConstraintsByText(1, B, self.real_x[self.time_num + step - 1], self.real_x[self.time_num + step - 1], num)


        self.real_x[self.time_num * 2 + step - 1] = self.x[self.time_num * 2 + step - 1]
        B = np.array([
            [self.name + "fm_u_g" + str(step), 1],
        ])
        CreatConstraintsByText(1, B, self.real_x[self.time_num * 2 + step - 1], self.real_x[self.time_num * 2 + step - 1], num)


        self.real_x[self.time_num * 3 + step - 1] = self.x[self.time_num * 3 + step - 1]
        B = np.array([
            [self.name + "fm_d_g" + str(step), 1],
        ])
        CreatConstraintsByText(1, B, self.real_x[self.time_num * 3 + step - 1],
                               self.real_x[self.time_num * 3 + step - 1], num)


        self.real_x[self.time_num * 4 + step - 1] = self.x[self.time_num * 4 + step - 1]
        B = np.array([
            [self.name + "fm_u_h" + str(step), 1],
        ])
        CreatConstraintsByText(1, B, self.real_x[self.time_num * 4 + step - 1],
                               self.real_x[self.time_num * 4 + step - 1], num)


        self.real_x[self.time_num * 5 + step - 1] = self.x[self.time_num * 5 + step - 1]
        B = np.array([
            [self.name + "fm_d_h" + str(step), 1],
        ])
        CreatConstraintsByText(1, B, self.real_x[self.time_num * 5 + step - 1],
                               self.real_x[self.time_num * 5 + step - 1], num)


        self.real_x[self.time_num * 6 + step - 1] = self.x[self.time_num * 6 + step - 1]
        B = np.array([
            [self.name + "fm_u_c" + str(step), 1],
        ])
        CreatConstraintsByText(1, B, self.real_x[self.time_num * 6 + step - 1],
                               self.real_x[self.time_num * 6 + step - 1], num)


        self.real_x[self.time_num * 7 + step - 1] = self.x[self.time_num * 7 + step - 1]
        B = np.array([
            [self.name + "fm_d_c" + str(step), 1],
        ])
        CreatConstraintsByText(1, B, self.real_x[self.time_num * 7 + step - 1],
                               self.real_x[self.time_num * 7 + step - 1], num)
