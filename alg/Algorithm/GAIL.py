import numpy as np
from Algorithm.Tool.network import Discriminator
import torch
...
class GAIL:
    ...
    def learn(self, expert_s, expert_a, agent_s, agent_a, next_s, dones):
        ...
        expert_actions = torch.LongTensor(expert_a).to(self.device)
        agent_states = torch.tensor(agent_s, dtype=torch.float).to(self.device)
        agent_actions = torch.LongTensor(agent_a).to(self.device)
