...
class AllegroHandOver(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        allegro_hand_another_asset_file = (
            "urdf/allegro_hand_model/urdf/allegro_hand_r.urdf"
        )
        ...
        self.num_allegro_hand_dof_dicts = self.gym.get_asset_dof_dict(
            allegro_hand_asset
        )
        ...
        allegro_another_hand_start_pose.r = gymapi.Quat().from_euler_zyx(
            -1.571, -1.571, 0
        )
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        self.obs_buf[
            :, action_another_obs_start : action_another_obs_start + 16
        ] = self.actions[:, 16:]
    ...
    def pre_physics_step(self, actions):
        ...
        if self.use_relative_control:
            ...
            self.cur_targets[:, self.actuated_dof_indices + 16] = scale(
                self.actions[:, 16:32],
                self.allegro_hand_dof_lower_limits[self.actuated_dof_indices],
                self.allegro_hand_dof_upper_limits[self.actuated_dof_indices],
            )
            self.cur_targets[:, self.actuated_dof_indices + 16] = (
                self.act_moving_average
                * self.cur_targets[:, self.actuated_dof_indices + 16]
                + (1.0 - self.act_moving_average)
                * self.prev_targets[:, self.actuated_dof_indices]
            )
            self.cur_targets[:, self.actuated_dof_indices + 16] = tensor_clamp(
                self.cur_targets[:, self.actuated_dof_indices + 16],
                self.allegro_hand_dof_lower_limits[self.actuated_dof_indices],
                self.allegro_hand_dof_upper_limits[self.actuated_dof_indices],
            )
        ...
        self.prev_targets[:, self.actuated_dof_indices + 16] = self.cur_targets[
            :, self.actuated_dof_indices + 16
        ]
