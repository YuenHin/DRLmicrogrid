...
class ShadowHandCatchAbreast(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        pose_dx, pose_dy, pose_dz = -0.39, 0.0, 0.04
    ...
    def compute_reward(self, actions):
        ...
        (
            ...
            self.left_hand_pos,
            self.right_hand_pos,
            self.rigid_body_states[:, 3 + 26, 0:3],
            self.rigid_body_states[:, 3, 0:3],
...
def compute_hand_reward(
    ...
    successes = torch.where(
        successes == 0,
        torch.where(goal_dist < 0.03, torch.ones_like(successes), successes),
        successes,
    )
    ...
    right_hand_base_dist = torch.norm(
        right_hand_base_pos
        - torch.tensor([-0.3, -0.55, 0.5], dtype=torch.float, device=device),
        p=2,
        dim=-1,
    )
    left_hand_base_dist = torch.norm(
        left_hand_base_pos
        - torch.tensor([-0.3, -1.15, 0.5], dtype=torch.float, device=device),
        p=2,
        dim=-1,
    )

    resets = torch.where(
        right_hand_base_dist >= 0.1, torch.ones_like(reset_buf), reset_buf
    )
