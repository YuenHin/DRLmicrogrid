        ...
        for i in range(self.num_envs):
            ...
            if self.obs_type in ["point_cloud"]:
                ...
                self.gym.set_camera_location(
                    camera_handle,
                    env_ptr,
                    gymapi.Vec3(0.4, -0.25, 1),
                    gymapi.Vec3(-0.4, -0.25, 0.25),
                )
...
def compute_hand_reward(
    ...
    right_hand_base_dist = torch.norm(
        right_hand_base_pos
        - torch.tensor([0.0, 0.0, 0.5], dtype=torch.float, device=device),
        p=2,
        dim=-1,
    )
    left_hand_base_dist = torch.norm(
        left_hand_base_pos
        - torch.tensor([0.0, -0.8, 0.5], dtype=torch.float, device=device),
        p=2,
        dim=-1,
    )
