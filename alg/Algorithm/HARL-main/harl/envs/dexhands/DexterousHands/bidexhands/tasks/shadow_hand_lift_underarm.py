...
class ShadowHandLiftUnderarm(BaseTask):
    ...
    def __init__(
        ...
        self.dof_force_tensor = gymtorch.wrap_tensor(dof_force_tensor).view(
            self.num_envs, self.num_shadow_hand_dofs * 2 + 2
        )
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        pose_dx, pose_dy, pose_dz = -0, -0.65, 0.0
        ...
        self.goal_displacement = gymapi.Vec3(-0.0, 0.0, 0.4)
    ...
    def compute_observations(self):
        ...
        self.pot_right_handle_pos = self.object_pos + quat_apply(
            self.object_rot,
            to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * 0.15,
        )
        self.pot_right_handle_pos = self.pot_right_handle_pos + quat_apply(
            self.object_rot,
            to_torch([0, 0, 1], device=self.device).repeat(self.num_envs, 1) * 0.06,
        )
        self.pot_left_handle_pos = self.object_pos + quat_apply(
            self.object_rot,
            to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * -0.15,
        )
        self.pot_left_handle_pos = self.pot_left_handle_pos + quat_apply(
            self.object_rot,
            to_torch([0, 0, 1], device=self.device).repeat(self.num_envs, 1) * 0.06,
        )
    ...
    def reset(self, env_ids, goal_env_ids):
        ...
        all_indices = torch.unique(
            torch.cat(
                [all_hand_indices, object_indices, self.table_indices[env_ids]]
            ).to(torch.int32)
        )
    ...
    def post_physics_step(self):
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                ...
                pot_left_handle_posx = (
                    (
                        self.pot_left_handle_pos[i]
                        + quat_apply(
                            self.object_rot[i],
                ...
                self.gym.add_lines(
                    ...
                    [
                        ...
                        p0[1],
                        p0[2],
                        pot_left_handle_posx[0],
                        pot_left_handle_posx[1],
                        pot_left_handle_posx[2],
                    ],
                    [0.85, 0.1, 0.1],
                )
                self.gym.add_lines(
                    ...
                    [
                        ...
                        p0[1],
                        p0[2],
                        pot_left_handle_posy[0],
                        pot_left_handle_posy[1],
                        pot_left_handle_posy[2],
                    ],
                    [0.1, 0.85, 0.1],
                )
                self.gym.add_lines(
                    ...
                    [
                        ...
                        p0[1],
                        p0[2],
                        pot_left_handle_posz[0],
                        pot_left_handle_posz[1],
                        pot_left_handle_posz[2],
                    ],
                    [0.1, 0.1, 0.85],
                )

                pot_right_handle_posx = (
                    (
                        self.pot_right_handle_pos[i]
                        + quat_apply(
                            self.object_rot[i],
                            to_torch([1, 0, 0], device=self.device) * 0.2,
                        )
                ...
                self.gym.add_lines(
                    ...
                    [
                        ...
                        p0[1],
                        p0[2],
                        pot_right_handle_posx[0],
                        pot_right_handle_posx[1],
                        pot_right_handle_posx[2],
                    ],
                    [0.85, 0.1, 0.1],
                )
                self.gym.add_lines(
                    ...
                    [
                        ...
                        p0[1],
                        p0[2],
                        pot_right_handle_posy[0],
                        pot_right_handle_posy[1],
                        pot_right_handle_posy[2],
                    ],
                    [0.1, 0.85, 0.1],
                )
                self.gym.add_lines(
                    ...
                    [
                        ...
                        p0[1],
                        p0[2],
                        pot_right_handle_posz[0],
                        pot_right_handle_posz[1],
                        pot_right_handle_posz[2],
                    ],
                    [0.1, 0.1, 0.85],
                )
                ...
                left_hand_posx = (
                    (
                        self.left_hand_pos[i]
                        + quat_apply(
                            self.left_hand_rot[i],
                            to_torch([1, 0, 0], device=self.device) * 0.2,
                        )
                    )
                    .cpu()
                ...
                right_hand_posx = (
                    (
                        self.right_hand_pos[i]
                        + quat_apply(
                            self.right_hand_rot[i],
                            to_torch([1, 0, 0], device=self.device) * 0.2,
                        )
                    )
                    .cpu()
...
def compute_hand_reward(
    ...
    up_rew = torch.where(
        right_hand_dist < 0.08,
        torch.where(left_hand_dist < 0.08, 3 * (0.385 - goal_dist), up_rew),
        up_rew,
    )
    ...
    successes = torch.where(
        successes == 0,
        torch.where(goal_dist < 0.05, torch.ones_like(successes), successes),
        successes,
    )
