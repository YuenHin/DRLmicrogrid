...
class ShadowHandReOrientation(BaseTask):
        ...
        for i in range(self.num_envs):
            ...
            if self.obs_type in ["point_cloud"]:
                ...
                self.gym.set_camera_location(
                    camera_handle,
                    env_ptr,
                    gymapi.Vec3(0.25, -0.575, 0.75),
                    gymapi.Vec3(-0.24, -0.575, 0),
                )
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        self.obs_buf[
            :, action_another_obs_start : action_another_obs_start + 20
        ] = self.actions[:, 20:]
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                self.add_debug_lines(
                    self.envs[i], self.object_pos[i], self.object_rot[i]
                )
                self.add_debug_lines(self.envs[i], self.goal_pos[i], self.goal_rot[i])
                self.add_debug_lines(
                    self.envs[i], self.object_another_pos[i], self.object_another_rot[i]
                )
                self.add_debug_lines(
                    self.envs[i], self.goal_another_pos[i], self.goal_another_rot[i]
                )
...
def compute_hand_reward(
    ...
    goal_resets = torch.where(
        torch.abs(rot_dist) < 0.1, torch.ones_like(reset_goal_buf), reset_goal_buf
    )
    goal_resets = torch.where(
        torch.abs(rot_another_dist) < 0.1,
