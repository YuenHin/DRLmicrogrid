...
class ShadowHandSwingCup(BaseTask):
    ...
    def compute_reward(self, actions):
        ...
        (
            ...
            self.goal_pos,
            self.goal_rot,
            self.cup_right_handle_pos,
            self.cup_left_handle_pos,
            self.left_hand_pos,
            self.right_hand_pos,
    ...
    def compute_observations(self):
        ...
        self.cup_right_handle_pos = self.cup_right_handle_pos + quat_apply(
            self.cup_right_handle_rot,
            to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * 0,
        )
        self.cup_right_handle_pos = self.cup_right_handle_pos + quat_apply(
            self.cup_right_handle_rot,
            to_torch([1, 0, 0], device=self.device).repeat(self.num_envs, 1) * 0.062,
        )
        self.cup_right_handle_pos = self.cup_right_handle_pos + quat_apply(
            self.cup_right_handle_rot,
            to_torch([0, 0, 1], device=self.device).repeat(self.num_envs, 1) * 0,
        )
        ...
        self.cup_left_handle_pos = self.cup_left_handle_pos + quat_apply(
            self.cup_left_handle_rot,
            to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * 0,
        )
        self.cup_left_handle_pos = self.cup_left_handle_pos + quat_apply(
            self.cup_left_handle_rot,
            to_torch([1, 0, 0], device=self.device).repeat(self.num_envs, 1) * 0,
        )
        self.cup_left_handle_pos = self.cup_left_handle_pos + quat_apply(
            self.cup_left_handle_rot,
    ...
    def reset_target_pose(self, env_ids, apply_reset=False):
        ...
        new_rot = quat_from_angle_axis(
            target_rot[:, 0] * 1.5 * np.pi, self.z_unit_tensor[env_ids]
        )
    ...
    def reset(self, env_ids, goal_env_ids):
        ...
        self.prev_targets[
            env_ids, self.num_shadow_hand_dofs * 2 : self.num_shadow_hand_dofs * 2 + 1
        ] = to_torch([0], device=self.device)
        self.cur_targets[
            env_ids, self.num_shadow_hand_dofs * 2 : self.num_shadow_hand_dofs * 2 + 1
        ] = to_torch([0], device=self.device)
        self.prev_targets[
            env_ids,
            self.num_shadow_hand_dofs * 2 + 1 : self.num_shadow_hand_dofs * 2 + 2 * 1,
        ] = to_torch([0], device=self.device)
        self.cur_targets[
            env_ids,
            self.num_shadow_hand_dofs * 2 + 1 : self.num_shadow_hand_dofs * 2 + 2 * 1,
...
def compute_hand_reward(
    ...
    up_rew = torch.where(
        right_hand_finger_dist < 0.4,
        torch.where(left_hand_finger_dist < 0.4, rot_rew, up_rew),
        up_rew,
    )
    ...
    successes = torch.where(
        successes == 0,
        torch.where(rot_dist < 0.785, torch.ones_like(successes), successes),
        successes,
    )
