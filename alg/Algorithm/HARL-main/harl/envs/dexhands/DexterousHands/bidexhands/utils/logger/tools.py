                ...
                for i, test_rew in enumerate(ea.scalars.Items("Train/mean_reward")):
                    content.append(
                        [
                            test_rew.step
                            * env_step
                            * env_num,  # if env is to lift a pot, change it as test_rew.step * 20 * 2048
...
def merge_csv(csv_files, root_dir, remove_zero=False):
    ...
    for rows in zip(*sorted_values):
        ...
        line = [
            round(array[:, 0].mean(), 4),
            round(array[:, 1].mean(), 4),
            round(array[:, 1].std(), 4),
        ]
        line += array[:, 1].tolist()
        content.append(line)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--alg-name", type=str, default="happo")
    parser.add_argument(
        "--alg-type",
        type=str,
        default="marl",
    ...
    parser.add_argument(
        "--env-num", type=int, default=2048, help="the number of parallel simulations"
    )
    parser.add_argument(
        "--env-step",
        type=int,
    ...
    csv_files = convert_tfevents_to_csv(
        args.root_dir, args.alg_type, args.env_num, args.env_step, args.refresh
    )
