            ...
            for i_episode in range(int(num_episodes / 10)):
                ...
                # if True:
                    ...
                    df = pd.DataFrame({
                        ...
                        'value': env.rl_res.x,
                        'milp_operation_cost': env.rl_res.fun,
                        'rl_total_cost': total_cost,
                        'rl_operation_cost': operation_cost
                    })
...
def train_off_policy_agent_MG_RO_notorch(env, agent, num_episodes, replay_buffer, minimal_size, batch_size, return_buffer, storage_punishment_buffer):
    action_list = []
    ave_num = int(num_episodes // 20)
    for i in range(10):
        with tqdm(total=int(num_episodes / 10), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes / 10)):
                ...
                while not done:
                    while not going:
                        ...
                        # action = agent.take_action_RO(state)
                        action = [1, 1]
                        next_state, reward, done, going, action_ = env.step3_notorch(action, cur_episode, storage_punishment_buffer)
                        # if next_state == None:
                        #     # next_state = state_1
                        action = action_
                        if going == True:
