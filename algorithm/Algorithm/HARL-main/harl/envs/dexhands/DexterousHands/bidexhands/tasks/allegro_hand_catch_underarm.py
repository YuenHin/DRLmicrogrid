...
class AllegroHandCatchUnderarm(BaseTask):
    def __init__(
        ...
        self.allegro_hand_default_dof_pos[:6] = torch.tensor(
            [0, 0, -1, 3.14, 0.57, 3.14], dtype=torch.float, device=self.device
        )
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        a_tendon_props = self.gym.get_asset_tendon_properties(
        ...
        x_arm_dof_effort = to_torch(
            [10, 10, 6, 6, 6, 4], dtype=torch.float, device=self.device
        )
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        self.obs_buf[:, 0 : self.num_allegro_hand_dofs] = unscale(
            self.allegro_hand_dof_pos,
            self.allegro_hand_dof_lower_limits,
            self.allegro_hand_dof_upper_limits,
        )
        self.obs_buf[:, self.num_allegro_hand_dofs : 2 * self.num_allegro_hand_dofs] = (
        ...
        self.obs_buf[
            :, another_hand_start : self.num_allegro_hand_dofs + another_hand_start
        ] = unscale(
        ...
        self.obs_buf[
            :,
            self.num_allegro_hand_dofs
            + another_hand_start : 2 * self.num_allegro_hand_dofs
            + another_hand_start,
        ] = (
        ...
        self.obs_buf[
            :, action_another_obs_start : action_another_obs_start + 22
        ] = self.actions[:, 22:]
    ...
    def reset(self, env_ids, goal_env_ids):
        ...
        # reset shadow hand
        delta_max = (
            self.allegro_hand_dof_upper_limits - self.allegro_hand_dof_default_pos
        )
        delta_min = (
            self.allegro_hand_dof_lower_limits - self.allegro_hand_dof_default_pos
        )
        rand_delta = (
            delta_min
            + (delta_max - delta_min)
        ...
        self.allegro_hand_dof_vel[env_ids, :] = (
            ...
            * rand_floats[
                :, 5 + self.num_allegro_hand_dofs : 5 + self.num_allegro_hand_dofs * 2
            ]
        )
    ...
    def pre_physics_step(self, actions):
        ...
        if self.use_relative_control:
            targets = (
                ...
                + self.allegro_hand_dof_speed_scale * self.dt * self.actions
            )
            ...
            targets = (
                self.prev_targets[:, :6]
                + self.allegro_hand_dof_speed_scale * self.dt * self.actions[:, :6]
            )
            self.cur_targets[:, :6] = tensor_clamp(
                targets,
                self.allegro_hand_dof_lower_limits[:6],
                self.allegro_hand_dof_upper_limits[:6],
            )
            ...
            self.cur_targets[:, self.actuated_dof_indices + 6] = tensor_clamp(
            ...
            targets = (
                self.prev_targets[:, 22:28]
                + self.allegro_hand_dof_speed_scale * self.dt * self.actions[:, 22:28]
            )
            self.cur_targets[:, 22:28] = tensor_clamp(
                targets,
                self.allegro_hand_dof_lower_limits[:6],
                self.allegro_hand_dof_upper_limits[:6],
            )
            ...
            self.cur_targets[:, self.actuated_dof_indices + 28] = tensor_clamp(
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                self.add_debug_lines(
                    self.envs[i],
                    self.allegro_right_hand_pos[i],
                    self.allegro_right_hand_rot[i],
                )
                self.add_debug_lines(
                    self.envs[i],
                    self.allegro_left_hand_pos[i],
                    self.allegro_left_hand_rot[i],
                )
...
def compute_hand_reward(
    ...
    resets = torch.where(
        allegro_right_hand_pos[:, 1] <= -0.8, torch.ones_like(resets), resets
    )
