...
class ShadowHandBlockStack(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        max_agg_bodies = (
            self.num_shadow_hand_bodies * 2 + 3 * self.num_object_bodies + 1
        )
        max_agg_shapes = (
            self.num_shadow_hand_shapes * 2 + 3 * self.num_object_shapes + 1
        )
        ...
        for i in range(self.num_envs):
            ...
            object_handle = self.gym.create_actor(
                env_ptr, block_asset, object_start_pose, "object", i, 0, 0
            )
            ...
            block_handle = self.gym.create_actor(
                env_ptr, block_asset, block_start_pose, "block", i, 0, 0
            )
            block_idx = self.gym.get_actor_index(
                env_ptr, block_handle, gymapi.DOMAIN_SIM
            )
            ...
            block_shape_props = self.gym.get_actor_rigid_shape_properties(
                env_ptr, block_handle
            )
            ...
            self.gym.set_actor_rigid_shape_properties(
                env_ptr, block_handle, block_shape_props
            )
            ...
            for object_shape_prop in another_hand_shape_props:
                object_shape_prop.friction = 0.5
            self.gym.set_actor_rigid_shape_properties(
                env_ptr, shadow_hand_actor, hand_shape_props
            )
            self.gym.set_actor_rigid_shape_properties(
            ...
            if self.obs_type in ["point_cloud"]:
                ...
                self.gym.set_camera_location(
                    camera_handle,
                    env_ptr,
                    gymapi.Vec3(0.25, 0, 1.0),
                    gymapi.Vec3(-0.25, 0, -0.5),
                )
        ...
        self.block_indices = to_torch(
            self.block_indices, dtype=torch.long, device=self.device
        )
    ...
    def compute_observations(self):
        ...
        self.block_right_handle_pos = self.block_right_handle_pos + quat_apply(
            self.block_right_handle_rot,
        ...
        self.block_left_handle_pos = self.block_left_handle_pos + quat_apply(
            self.block_left_handle_rot,
            to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * 0.0,
        )
        self.block_left_handle_pos = self.block_left_handle_pos + quat_apply(
            self.block_left_handle_rot,
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        self.obs_buf[
            :, obj_obs_start + 13 + 7 : obj_obs_start + 13 + 10
        ] = self.block_linvel
        self.obs_buf[:, obj_obs_start + 13 + 10 : obj_obs_start + 13 + 13] = (
            self.vel_obs_scale * self.block_angvel
        )
    ...
    def compute_point_cloud_observation(self, collect_demonstration=False):
        ...
        self.obs_buf[
            :,
            point_clouds_start : point_clouds_start + self.pointCloudDownsampleNum * 3,
        ].copy_(point_clouds.view(self.num_envs, self.pointCloudDownsampleNum * 3))
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                self.add_debug_lines(
                    self.envs[i],
                    self.block_right_handle_pos[i],
                    self.block_right_handle_rot[i],
                )
                self.add_debug_lines(
                    self.envs[i],
                    self.block_left_handle_pos[i],
                    self.block_left_handle_rot[i],
                )
...
def compute_hand_reward(
    ...
    right_hand_finger_dist = (
        torch.norm(block_right_handle_pos - right_hand_ff_pos, p=2, dim=-1)
    ...
    left_hand_finger_dist = (
        torch.norm(block_left_handle_pos - left_hand_ff_pos, p=2, dim=-1)
    ...
    up_rew = torch.where(
        ...
        torch.where(
            left_hand_finger_dist < 0.5, (0.24 - goal_dist1 - goal_dist2) * 2, up_rew
        ),
        up_rew,
    )
    ...
    resets = torch.where(
        right_hand_finger_dist >= 0.75, torch.ones_like(resets), resets
    )
    ...
    successes = torch.where(
        successes == 0,
        torch.where(stack_rew > 1, torch.ones_like(successes), successes),
        successes,
    )
