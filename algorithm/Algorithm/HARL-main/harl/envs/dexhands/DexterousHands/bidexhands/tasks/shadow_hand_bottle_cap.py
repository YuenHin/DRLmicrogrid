...
class ShadowHandBottleCap(BaseTask):
    ...
    def __init__(
        ...
        self.shadow_hand_default_dof_pos = to_torch(
            [
                0.0,
                0.0,
                -0,
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        self.object_dof_default_pos = to_torch(
            self.object_dof_default_pos, device=self.device
        )
        self.object_dof_default_vel = to_torch(
            self.object_dof_default_vel, device=self.device
        )
        ...
        for i in range(self.num_envs):
            ...
            if self.obs_type in ["point_cloud"]:
                ...
                self.gym.set_camera_location(
                    camera_handle,
                    env_ptr,
                    gymapi.Vec3(0.25, -0.6, 0.75),
                    gymapi.Vec3(-0.5, -0.6, 0),
                )
    ...
    def compute_observations(self):
        ...
        self.bottle_pos = self.object_pos + quat_apply(
            self.object_rot,
            to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * 0.0,
        )
        self.bottle_pos = self.bottle_pos + quat_apply(
            self.object_rot,
            to_torch([0, 0, 1], device=self.device).repeat(self.num_envs, 1) * 0.0,
        )

        self.bottle_cap_up = self.rigid_body_states[:, 26 * 2 + 3, 0:3].clone()
        self.bottle_cap_pos = self.object_pos + quat_apply(
            self.object_rot,
            to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * -0.0,
        )
        self.bottle_cap_pos = self.bottle_cap_pos + quat_apply(
            self.object_rot,
            to_torch([0, 0, 1], device=self.device).repeat(self.num_envs, 1) * 0.15,
        )
    ...
    def reset(self, env_ids, goal_env_ids):
        ...
        self.hand_linvels[all_hand_indices.to(torch.long), :] = torch.zeros_like(
            self.saved_root_tensor[all_hand_indices.to(torch.long), 7:10]
        )
        self.hand_angvels[all_hand_indices.to(torch.long), :] = torch.zeros_like(
            self.saved_root_tensor[all_hand_indices.to(torch.long), 10:13]
        )
...
def compute_hand_reward(
    ...
    right_hand_finger_dist = (
        torch.norm(bottle_cap_pos - right_hand_ff_pos, p=2, dim=-1)
    ...
    up_rew = torch.where(
        right_hand_finger_dist <= 0.3,
        torch.norm(bottle_cap_up - bottle_pos, p=2, dim=-1) * 30,
        up_rew,
    )
    ...
    resets = torch.where(
        bottle_cap_pos[:, 2] <= 0.5, torch.ones_like(reset_buf), reset_buf
    )
    ...
    successes = torch.where(
        successes == 0,
        torch.where(
            torch.norm(bottle_cap_up - bottle_pos, p=2, dim=-1) > 0.03,
