...
class ShadowHandDoorCloseInward(BaseTask):
    ...
    def __init__(
        ...
        self.dof_force_tensor = gymtorch.wrap_tensor(dof_force_tensor).view(
            self.num_envs, self.num_shadow_hand_dofs * 2 + 4
        )
        ...
        self.goal_object_dof_state = self.dof_state.view(self.num_envs, -1, 2)[
            :,
            self.num_shadow_hand_dofs * 2
            + self.num_object_dofs : self.num_shadow_hand_dofs * 2
            + 2 * self.num_object_dofs,
        ]
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # create table asset
        table_dims = gymapi.Vec3(0.3, 0.3, 0.0)
        asset_options = gymapi.AssetOptions()
        ...
        object_start_pose.p = gymapi.Vec3(0.0, 0.0, 0.7)
        ...
        self.goal_displacement = gymapi.Vec3(-0.0, 0.0, 5.0)
        ...
        max_agg_bodies = (
            self.num_shadow_hand_bodies * 2 + 2 * self.num_object_bodies + 1
        )
        max_agg_shapes = (
            self.num_shadow_hand_shapes * 2 + 2 * self.num_object_shapes + 1
        )
    ...
    def compute_observations(self):
        ...
        self.door_left_handle_pos = self.door_left_handle_pos + quat_apply(
            self.door_left_handle_rot,
    ...
    def reset(self, env_ids, goal_env_ids):
        ...
        self.goal_object_dof_pos[env_ids, :] = to_torch(
            [1.57, 1.57], device=self.device
        )
        self.object_dof_vel[env_ids, :] = to_torch([1.57, 1.57], device=self.device)
        self.goal_object_dof_vel[env_ids, :] = to_torch(
            [1.57, 1.57], device=self.device
        )
        ...
        self.prev_targets[
            env_ids, self.num_shadow_hand_dofs * 2 : self.num_shadow_hand_dofs * 2 + 2
        ] = to_torch([1.57, 1.57], device=self.device)
        self.cur_targets[
            env_ids, self.num_shadow_hand_dofs * 2 : self.num_shadow_hand_dofs * 2 + 2
        ] = to_torch([1.57, 1.57], device=self.device)
        self.prev_targets[
            env_ids,
            self.num_shadow_hand_dofs * 2 + 2 : self.num_shadow_hand_dofs * 2 + 2 * 2,
        ] = to_torch([1.57, 1.57], device=self.device)
        self.cur_targets[
            env_ids,
            self.num_shadow_hand_dofs * 2 + 2 : self.num_shadow_hand_dofs * 2 + 2 * 2,
        ...
        all_hand_indices = torch.unique(
            torch.cat([hand_indices, another_hand_indices, object_indices]).to(
                torch.int32
            )
        )
        ...
        self.hand_linvels[all_hand_indices.to(torch.long), :] = self.saved_root_tensor[
            all_hand_indices.to(torch.long), 7:10
        ]
        self.hand_angvels[all_hand_indices.to(torch.long), :] = self.saved_root_tensor[
            all_hand_indices.to(torch.long), 10:13
        ]
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                self.add_debug_lines(
                    self.envs[i],
                    self.door_left_handle_pos[i],
                    self.door_left_handle_rot[i],
                )
                self.add_debug_lines(
                    self.envs[i],
                    self.door_right_handle_pos[i],
                    self.door_right_handle_rot[i],
                )

                self.add_debug_lines(
                    self.envs[i], self.right_hand_ff_pos[i], self.right_hand_ff_rot[i]
                )
                self.add_debug_lines(
                    self.envs[i], self.right_hand_mf_pos[i], self.right_hand_mf_rot[i]
                )
                self.add_debug_lines(
                    self.envs[i], self.right_hand_rf_pos[i], self.right_hand_rf_rot[i]
                )
                self.add_debug_lines(
                    self.envs[i], self.right_hand_lf_pos[i], self.right_hand_lf_rot[i]
                )
                self.add_debug_lines(
                    self.envs[i], self.right_hand_th_pos[i], self.right_hand_th_rot[i]
                )

                self.add_debug_lines(
                    self.envs[i], self.left_hand_ff_pos[i], self.right_hand_ff_rot[i]
                )
                self.add_debug_lines(
                    self.envs[i], self.left_hand_mf_pos[i], self.right_hand_mf_rot[i]
                )
                self.add_debug_lines(
                    self.envs[i], self.left_hand_rf_pos[i], self.right_hand_rf_rot[i]
                )
                self.add_debug_lines(
                    self.envs[i], self.left_hand_lf_pos[i], self.right_hand_lf_rot[i]
                )
                self.add_debug_lines(
                    self.envs[i], self.left_hand_th_pos[i], self.right_hand_th_rot[i]
                )
...
def compute_hand_reward(
    ...
    up_rew = torch.where(
        ...
        torch.where(
            ...
            1 - torch.abs(door_right_handle_pos[:, 1] - door_left_handle_pos[:, 1]) * 2,
            up_rew,
        ),
        up_rew,
    )
    ...
    resets = torch.where(
        right_hand_finger_dist >= 1.5, torch.ones_like(reset_buf), reset_buf
    )
