...
class ShadowHandMetaMT4(BaseTask):
    def __init__(
        ...
        if self.obs_type == "full_state" or self.asymmetric_obs:
            ...
            self.dof_force_tensor = torch.zeros(
                (self.num_envs, 48), dtype=torch.float, device=self.device
            )
            pointer = 0
            for i in range(self.num_envs):
                self.dof_force_tensor[i, :] = tem_dof_force_tensor[
                    pointer : pointer + 48
                ]
        ...
        tem_shadow_hand_dof = torch.zeros(
            (self.num_envs, 48, 2), dtype=torch.float, device=self.device
        )
        pointer = 0
        for i in range(self.num_envs):
        ...
        self.shadow_hand_another_dof_state = tem_shadow_hand_dof[
        ...
        self.rigid_body_states = torch.zeros(
            (self.num_envs, 52, 13), dtype=torch.float, device=self.device
        )
        pointer = 0
        for i in range(self.num_envs):
            self.rigid_body_states[i, :, :] = self.total_rigid_body_tensor[
                pointer : pointer + 52, :
            ]
        ...
        self.apply_forces = torch.zeros(
            (self.num_envs, 52, 3), device=self.device, dtype=torch.float
        )
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for task_env in self.task_envs:
            ...
            shadow_hand_start_pose.p = gymapi.Vec3(
                hand_start_pose[0], hand_start_pose[1], hand_start_pose[2]
            )
            shadow_hand_start_pose.r = gymapi.Quat().from_euler_zyx(
                hand_start_pose[3], hand_start_pose[4], hand_start_pose[5]
            )

            shadow_another_hand_start_pose = gymapi.Transform()
            shadow_another_hand_start_pose.p = gymapi.Vec3(
                another_hand_start_pose[0],
                another_hand_start_pose[1],
                another_hand_start_pose[2],
            )
            shadow_another_hand_start_pose.r = gymapi.Quat().from_euler_zyx(
                another_hand_start_pose[3],
            ...
            object_start_pose.r = gymapi.Quat().from_euler_zyx(
                object_pose[3], object_pose[4], object_pose[5]
            )

            goal_start_pose = gymapi.Transform()
            goal_start_pose.p = gymapi.Vec3(goal_pose[0], goal_pose[1], goal_pose[2])
            goal_start_pose.r = gymapi.Quat().from_euler_zyx(
                goal_pose[3], goal_pose[4], goal_pose[5]
            )

            another_object_start_pose = gymapi.Transform()
            another_object_start_pose.p = gymapi.Vec3(
                another_object_pose[0], another_object_pose[1], another_object_pose[2]
            )
            another_object_start_pose.r = gymapi.Quat().from_euler_zyx(
                another_object_pose[3], another_object_pose[4], another_object_pose[5]
            )

            another_goal_start_pose = gymapi.Transform()
            another_goal_start_pose.p = gymapi.Vec3(
                another_goal_pose[0], another_goal_pose[1], another_goal_pose[2]
            )
            another_goal_start_pose.r = gymapi.Quat().from_euler_zyx(
                another_goal_pose[3], another_goal_pose[4], another_goal_pose[5]
            )

            # create table asset
            table_dims = gymapi.Vec3(
                table_pose_dim[6], table_pose_dim[7], table_pose_dim[8]
            )
            asset_options = gymapi.AssetOptions()
            ...
            table_pose.p = gymapi.Vec3(
                table_pose_dim[0], table_pose_dim[1], table_pose_dim[2]
            )
            table_pose.r = gymapi.Quat().from_euler_zyx(
                table_pose_dim[3], table_pose_dim[4], table_pose_dim[5]
            )
            ...
            object_assets.append(
                self.gym.load_asset(
                    self.sim,
                    asset_root,
                    self.asset_files_dict[object_type],
                    object_asset_options,
                )
            )
            goal_assets.append(
            ...
            another_object_assets.append(
                self.gym.load_asset(
                    self.sim,
                    asset_root,
                    self.asset_files_dict[object_type],
                    another_object_asset_options,
                )
            )
            another_goal_assets.append(
                self.gym.load_asset(
                    self.sim,
                    asset_root,
                    self.asset_files_dict[object_type],
        ...
        for env_id in range(self.num_tasks):
            for i in range(self.num_each_envs):
                ...
                shadow_hand_actor = self.gym.create_actor(
                    env_ptr,
                    shadow_hand_asset,
                    shadow_hand_start_poses[env_id],
                    "hand",
                    index,
                    0,
                    0,
                ...
                self.hand_start_states.append(
                    [
                        shadow_hand_start_poses[env_id].p.x,
                        shadow_hand_start_poses[env_id].p.y,
                        shadow_hand_start_poses[env_id].p.z,
                        shadow_hand_start_poses[env_id].r.x,
                        shadow_hand_start_poses[env_id].r.y,
                        shadow_hand_start_poses[env_id].r.z,
                        shadow_hand_start_poses[env_id].r.w,
                        0,
                        0,
                        0,
                        0,
                ...
                object_handle = self.gym.create_actor(
                    env_ptr,
                    object_assets[env_id],
                    object_start_poses[env_id],
                    "object",
                    index,
                    0,
                    0,
                ...
                self.object_init_state.append(
                    [
                        object_start_poses[env_id].p.x,
                        object_start_poses[env_id].p.y,
                        object_start_poses[env_id].p.z,
                        object_start_poses[env_id].r.x,
                        object_start_poses[env_id].r.y,
                        object_start_poses[env_id].r.z,
                ...
                self.object_dof.append(
                    self.gym.get_asset_dof_count(object_assets[env_id]) * 2
                )
                self.object_rigid_body.append(
                    self.gym.get_asset_rigid_body_count(object_assets[env_id]) * 2
                )
                ...
                self.goal_init_state.append(
                    [
                        goal_start_poses[env_id].p.x,
                        goal_start_poses[env_id].p.y,
                        goal_start_poses[env_id].p.z,
                        goal_start_poses[env_id].r.x,
                        goal_start_poses[env_id].r.y,
                        goal_start_poses[env_id].r.z,
                ...
                # add another object
                object_another_handle = self.gym.create_actor(
                    env_ptr,
                    another_object_assets[env_id],
                    another_object_start_poses[env_id],
                    "another_object",
                    index,
                    0,
                    0,
                )
                self.another_object_init_state.append(
                    [
                        another_object_start_poses[env_id].p.x,
                        another_object_start_poses[env_id].p.y,
                        another_object_start_poses[env_id].p.z,
                        another_object_start_poses[env_id].r.x,
                        another_object_start_poses[env_id].r.y,
                        another_object_start_poses[env_id].r.z,
                        another_object_start_poses[env_id].r.w,
                        0,
                        0,
                        0,
                        0,
                ...
                object_another_idx = self.gym.get_actor_index(
                    env_ptr, object_another_handle, gymapi.DOMAIN_SIM
                )
                self.another_object_indices.append(object_another_idx)

                # add another goal object
                goal_another_handle = self.gym.create_actor(
                    env_ptr,
                    another_goal_assets[env_id],
                    another_goal_start_poses[env_id],
                    "another_goal_object",
                    index + self.num_envs * 2,
                    0,
                    0,
                )
                self.another_goal_init_state.append(
                    [
                        another_goal_start_poses[env_id].p.x,
                        another_goal_start_poses[env_id].p.y,
                        another_goal_start_poses[env_id].p.z,
                        another_goal_start_poses[env_id].r.x,
                        another_goal_start_poses[env_id].r.y,
                        another_goal_start_poses[env_id].r.z,
                        another_goal_start_poses[env_id].r.w,
                        0,
                        0,
                        0,
                        0,
                ...
                another_goal_object_idx = self.gym.get_actor_index(
                    env_ptr, goal_another_handle, gymapi.DOMAIN_SIM
                )
        ...
        self.goal_init_state = to_torch(
            self.goal_init_state, device=self.device, dtype=torch.float
        ).view(self.num_envs, 13)
        self.another_object_init_state = to_torch(
            self.another_object_init_state, device=self.device, dtype=torch.float
        ).view(self.num_envs, 13)
        self.another_goal_init_state = to_torch(
            self.another_goal_init_state, device=self.device, dtype=torch.float
        ).view(self.num_envs, 13)
        ...
        self.another_object_indices = to_torch(
            self.another_object_indices, dtype=torch.long, device=self.device
        )
        self.another_goal_object_indices = to_torch(
            self.another_goal_object_indices, dtype=torch.long, device=self.device
        )
    ...
    def compute_reward(self, actions):
        (
            ...
            self.goal_pos,
            self.goal_rot,
            self.another_object_pos,
            self.another_object_rot,
            self.another_goal_pos,
            self.another_goal_rot,
            self.object_left_handle_pos,
            self.object_right_handle_pos,
            self.left_hand_pos,
            self.right_hand_pos,
    ...
    def compute_observations(self):
        ...
        self.another_object_pose = self.root_state_tensor[
            self.another_object_indices, 0:7
        ]
        self.another_object_pos = self.root_state_tensor[
            self.another_object_indices, 0:3
        ]
        self.another_object_rot = self.root_state_tensor[
            self.another_object_indices, 3:7
        ]
        self.another_object_linvel = self.root_state_tensor[
            self.another_object_indices, 7:10
        ]
        self.another_object_angvel = self.root_state_tensor[
            self.another_object_indices, 10:13
        ]
        ...
        self.object_left_handle_pos = self.object_left_handle_pos + quat_apply(
            self.object_left_handle_rot,
        ...
        self.object_right_handle_pos = self.object_right_handle_pos + quat_apply(
            self.object_right_handle_rot,
            to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * -0.5,
        )
        self.object_right_handle_pos = self.object_right_handle_pos + quat_apply(
            self.object_right_handle_rot,
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        self.obs_buf[
            :, another_obj_obs_start : another_obj_obs_start + 7
        ] = self.another_object_pose
        self.obs_buf[
            :, another_obj_obs_start + 7 : another_obj_obs_start + 10
        ] = self.another_object_linvel
        self.obs_buf[:, another_obj_obs_start + 10 : another_obj_obs_start + 13] = (
            self.vel_obs_scale * self.another_object_angvel
        )
    ...
    def reset_target_pose(self, env_ids, apply_reset=False):
        ...
        self.another_goal_states[env_ids, 0:3] = self.another_goal_init_state[
            env_ids, 0:3
        ]
        ...
        self.root_state_tensor[
            self.another_goal_object_indices[env_ids], 0:3
        ] = self.another_goal_states[env_ids, 0:3]
        self.root_state_tensor[
            self.another_goal_object_indices[env_ids], 3:7
        ] = self.another_goal_states[env_ids, 3:7]
        self.root_state_tensor[
            self.another_goal_object_indices[env_ids], 7:13
        ] = torch.zeros_like(
            self.root_state_tensor[self.another_goal_object_indices[env_ids], 7:13]
        )
    ...
    def reset(self, env_ids, goal_env_ids):
        ...
        ] = self.another_object_init_state[env_ids].clone()
        ...
        self.total_dof_target = torch.zeros(
            self.num_total_dof, dtype=torch.float, device=self.device
        )
        pointer = 0
        for i in range(self.num_envs):
        ...
        self.hand_positions[
            all_hand_indices.to(torch.long), :
        ] = self.saved_root_tensor[all_hand_indices.to(torch.long), 0:3]
        self.hand_orientations[
            all_hand_indices.to(torch.long), :
        ] = self.saved_root_tensor[all_hand_indices.to(torch.long), 3:7]
    ...
    def pre_physics_step(self, actions):
        ...
        if self.use_relative_control:
            ...
            self.total_apply_force = torch.zeros(
                (self.num_total_rigid_body, 3), dtype=torch.float, device=self.device
            )
            self.total_apply_torque = torch.zeros(
                (self.num_total_rigid_body, 3), dtype=torch.float, device=self.device
            )
            pointer = 0
            for i in range(self.num_envs):
            ...
            self.gym.apply_rigid_body_force_tensors(
                self.sim,
                gymtorch.unwrap_tensor(self.total_apply_force),
                gymtorch.unwrap_tensor(self.total_apply_torque),
                gymapi.ENV_SPACE,
            )
        ...
        self.gym.set_dof_position_target_tensor(
            self.sim, gymtorch.unwrap_tensor(self.total_dof_target)
        )
