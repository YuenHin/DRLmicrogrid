...
class ShadowHandPen(BaseTask):
    ...
    def compute_observations(self):
        ...
        self.pen_right_handle_pos = self.pen_right_handle_pos + quat_apply(
            self.pen_right_handle_rot,
            to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * -0.1,
        )
        self.pen_right_handle_pos = self.pen_right_handle_pos + quat_apply(
            self.pen_right_handle_rot,
        ...
        self.pen_left_handle_pos = self.pen_left_handle_pos + quat_apply(
            self.pen_left_handle_rot,
            to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * 0.07,
        )
        self.pen_left_handle_pos = self.pen_left_handle_pos + quat_apply(
            self.pen_left_handle_rot,
            to_torch([1, 0, 0], device=self.device).repeat(self.num_envs, 1) * 0.0,
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                self.add_debug_lines(
                    self.envs[i],
                    self.pen_right_handle_pos[i],
                    self.pen_right_handle_rot[i],
                )
                self.add_debug_lines(
                    self.envs[i],
                    self.pen_left_handle_pos[i],
                    self.pen_left_handle_rot[i],
                )
...
def compute_hand_reward(
    ...
    up_rew = torch.where(
        right_hand_finger_dist < 0.75,
        torch.where(
            left_hand_finger_dist < 0.75,
            torch.norm(pen_right_handle_pos - pen_left_handle_pos, p=2, dim=-1) * 5
            - 0.8,
            up_rew,
        ),
        up_rew,
