    ...
    def post_physics_step(self):
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                ...
                self.add_debug_lines(
                    self.envs[i], self.left_goal_pos[i], self.block_left_handle_rot[i]
                )
                self.add_debug_lines(
                    self.envs[i], self.right_goal_pos[i], self.block_left_handle_rot[i]
                )
...
def compute_hand_reward(
    ...
    resets = torch.where(
        right_hand_finger_dist >= 1.2, torch.ones_like(reset_buf), reset_buf
    )
    ...
    successes = torch.where(
        successes == 0,
        torch.where(
            torch.abs(left_goal_dist) <= 0.1,
            torch.where(
                torch.abs(right_goal_dist) <= 0.1,
                torch.ones_like(successes),
                torch.ones_like(successes) * 0.5,
            ),
            successes,
        ),
        successes,
