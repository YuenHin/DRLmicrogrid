...
class ShadowHandScissors(BaseTask):
    ...
    def compute_reward(self, actions):
        ...
        (
            ...
            self.goal_pos,
            self.goal_rot,
            self.scissors_right_handle_pos,
            self.scissors_left_handle_pos,
            self.object_dof_pos,
            self.left_hand_pos,
            self.right_hand_pos,
    ...
    def compute_observations(self):
        ...
        self.scissors_right_handle_pos = self.scissors_right_handle_pos + quat_apply(
            self.scissors_right_handle_rot,
        ...
        self.scissors_left_handle_pos = self.scissors_left_handle_pos + quat_apply(
            self.scissors_left_handle_rot,
            to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * 0.0,
        )
        self.scissors_left_handle_pos = self.scissors_left_handle_pos + quat_apply(
            self.scissors_left_handle_rot,
    ...
    def reset(self, env_ids, goal_env_ids):
        ...
        ] = to_torch([-0.59], device=self.device)
        self.cur_targets[
            env_ids, self.num_shadow_hand_dofs * 2 : self.num_shadow_hand_dofs * 2 + 2
        ] = to_torch([-0.59], device=self.device)
        ...
        all_hand_indices = torch.unique(
            torch.cat(
                [hand_indices, another_hand_indices, self.object_indices[env_ids]]
            ).to(torch.int32)
        )
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                self.add_debug_lines(
                    self.envs[i],
                    self.scissors_right_handle_pos[i],
                    self.scissors_right_handle_rot[i],
                )
                self.add_debug_lines(
                    self.envs[i],
                    self.scissors_left_handle_pos[i],
                    self.scissors_left_handle_rot[i],
                )
...
def compute_hand_reward(
    ...
    right_hand_dist = torch.norm(
        scissors_right_handle_pos - right_hand_pos, p=2, dim=-1
    )
    left_hand_dist = torch.norm(scissors_left_handle_pos - left_hand_pos, p=2, dim=-1)

    right_hand_finger_dist = (
        torch.norm(scissors_right_handle_pos - right_hand_ff_pos, p=2, dim=-1)
    ...
    left_hand_finger_dist = (
        torch.norm(scissors_left_handle_pos - left_hand_ff_pos, p=2, dim=-1)
    ...
    up_rew = torch.where(
        ...
        torch.where(
            left_hand_finger_dist < 0.7, (0.59 + object_dof_pos[:, 0]) * 5, up_rew
        ),
        up_rew,
    )
    ...
    resets = torch.where(
        right_hand_finger_dist >= 1.75, torch.ones_like(resets), resets
    )
