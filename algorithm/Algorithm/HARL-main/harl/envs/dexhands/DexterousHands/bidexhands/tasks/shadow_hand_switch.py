...
class ShadowHandSwitch(BaseTask):
    ...
    def __init__(
        ...
        self.dof_force_tensor = gymtorch.wrap_tensor(dof_force_tensor).view(
            self.num_envs, self.num_shadow_hand_dofs * 2 + self.num_object_dofs * 2 + 1
        )
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_envs):
            ...
            block_handle = self.gym.create_actor(
                env_ptr, object_asset, block_start_pose, "switch", i, 0, 0
            )
    ...
    def compute_reward(self, actions):
        ...
        (
            ...
            self.goal_pos,
            self.goal_rot,
            self.switch_right_handle_pos,
            self.switch_left_handle_pos,
            self.left_hand_pos,
            self.right_hand_pos,
    ...
    def compute_observations(self):
        ...
        self.switch_right_handle_pos = self.switch_right_handle_pos + quat_apply(
            self.switch_right_handle_rot,
            to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * -0.02,
        )
        self.switch_right_handle_pos = self.switch_right_handle_pos + quat_apply(
            self.switch_right_handle_rot,
            to_torch([1, 0, 0], device=self.device).repeat(self.num_envs, 1) * -0.05,
        )
        self.switch_right_handle_pos = self.switch_right_handle_pos + quat_apply(
            self.switch_right_handle_rot,
        ...
        self.switch_left_handle_pos = self.switch_left_handle_pos + quat_apply(
            self.switch_left_handle_rot,
            to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * -0.02,
        )
        self.switch_left_handle_pos = self.switch_left_handle_pos + quat_apply(
            self.switch_left_handle_rot,
    ...
    def reset(self, env_ids, goal_env_ids):
        ...
        all_hand_indices = torch.unique(
            torch.cat([hand_indices, another_hand_indices, self.object_indices]).to(
                torch.int32
            )
        )
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                self.add_debug_lines(
                    self.envs[i],
                    self.switch_right_handle_pos[i],
                    self.switch_right_handle_rot[i],
                )
                self.add_debug_lines(
                    self.envs[i],
                    self.switch_left_handle_pos[i],
                    self.switch_left_handle_rot[i],
                )
...
def compute_hand_reward(
    ...
    right_hand_finger_dist = (
        torch.norm(switch_right_handle_pos - right_hand_ff_pos, p=2, dim=-1)
    ...
    left_hand_finger_dist = (
        torch.norm(switch_left_handle_pos - left_hand_ff_pos, p=2, dim=-1)
    ...
    successes = torch.where(
        successes == 0,
        torch.where(
            1.4 - (switch_right_handle_pos[:, 2] + switch_left_handle_pos[:, 2]) > 0.05,
