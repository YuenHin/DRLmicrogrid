...
class ShadowHandTwoCatchUnderarm(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        object_another_asset = self.gym.load_asset(
            self.sim, asset_root, object_asset_file, object_asset_options
        )
        ...
        goal_another_asset = self.gym.load_asset(
            self.sim, asset_root, object_asset_file, object_asset_options
        )
        ...
        if self.obs_type in ["point_cloud"]:
            ...
            if self.point_cloud_debug:
                ...
                from harl.envs.dexhands.DexterousHands.bidexhands.utils.o3dviewer import (
        ...
        for i in range(self.num_envs):
            ...
            object_another_handle = self.gym.create_actor(
                env_ptr,
                object_another_asset,
                object_another_start_pose,
                "another_object",
                i,
                0,
                0,
            )
            self.object_another_init_state.append(
                [
                    object_another_start_pose.p.x,
                    object_another_start_pose.p.y,
                    object_another_start_pose.p.z,
                    object_another_start_pose.r.x,
                    object_another_start_pose.r.y,
                    object_another_start_pose.r.z,
                    object_another_start_pose.r.w,
                    0,
                    0,
                    0,
                    0,
            ...
            goal_another_handle = self.gym.create_actor(
                env_ptr,
                goal_another_asset,
                goal_another_start_pose,
                "another_goal_object",
                i + self.num_envs,
                0,
                0,
            )
            goal_another_object_idx = self.gym.get_actor_index(
            ...
            if self.object_type != "block":
                ...
                self.gym.set_rigid_body_color(
                    env_ptr,
                    object_another_handle,
                    0,
                    gymapi.MESH_VISUAL,
                    gymapi.Vec3(0.98, 0.6, 0.72),
                )
                self.gym.set_rigid_body_color(
                    env_ptr,
                    goal_another_handle,
        ...
        self.object_another_init_state = to_torch(
            self.object_another_init_state, device=self.device, dtype=torch.float
        ).view(self.num_envs, 13)
        ...
        self.object_another_indices = to_torch(
            self.object_another_indices, dtype=torch.long, device=self.device
        )
        self.goal_object_another_indices = to_torch(
            self.goal_object_another_indices, dtype=torch.long, device=self.device
        )
    ...
    def compute_reward(self, actions):
        ...
        (
            ...
            self.goal_pos,
            self.goal_rot,
            self.object_another_pos,
            self.object_another_rot,
    ...
    def compute_observations(self):
        ...
        self.object_another_pose = self.root_state_tensor[
            self.object_another_indices, 0:7
        ]
        self.object_another_pos = self.root_state_tensor[
            self.object_another_indices, 0:3
        ]
        self.object_another_rot = self.root_state_tensor[
            self.object_another_indices, 3:7
        ]
        self.object_another_linvel = self.root_state_tensor[
            self.object_another_indices, 7:10
        ]
        self.object_another_angvel = self.root_state_tensor[
            self.object_another_indices, 10:13
        ]
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        ] = self.object_another_pose
        self.obs_buf[
            :, another_obj_obs_start + 7 : another_obj_obs_start + 10
        ] = self.object_another_linvel
        self.obs_buf[:, another_obj_obs_start + 10 : another_obj_obs_start + 13] = (
            self.vel_obs_scale * self.object_another_angvel
        )

        another_goal_obs_start = another_obj_obs_start + 13  # 157 = 144 + 13
        self.obs_buf[
            :, another_goal_obs_start : another_goal_obs_start + 7
        ] = self.goal_another_pose
        self.obs_buf[
            :, another_goal_obs_start + 7 : another_goal_obs_start + 11
        ] = quat_mul(self.object_another_rot, quat_conjugate(self.goal_another_rot))
    ...
    def reset_target_pose(self, env_ids, apply_reset=False):
        ...
        self.goal_another_states[env_ids, 0:3] = self.goal_another_init_state[
            env_ids, 0:3
        ]
        ...
        self.root_state_tensor[self.goal_object_another_indices[env_ids], 0:3] = (
            self.goal_another_states[env_ids, 0:3] + self.goal_displacement_tensor
        )
        self.root_state_tensor[
            self.goal_object_another_indices[env_ids], 3:7
        ] = self.goal_another_states[env_ids, 3:7]
        self.root_state_tensor[
            self.goal_object_another_indices[env_ids], 7:13
        ] = torch.zeros_like(
            self.root_state_tensor[self.goal_object_another_indices[env_ids], 7:13]
        )

        if apply_reset:
            goal_object_indices = self.goal_object_indices[env_ids].to(torch.int32)
            goal_object_another_indices = self.goal_object_another_indices[env_ids].to(
                torch.int32
            )
            ...
            self.gym.set_actor_root_state_tensor_indexed(
                self.sim,
                gymtorch.unwrap_tensor(self.root_state_tensor),
                gymtorch.unwrap_tensor(goal_object_another_indices),
                len(env_ids),
            )
    ...
    def reset(self, env_ids, goal_env_ids):
        ...
        ] = self.object_another_init_state[env_ids].clone()
        self.root_state_tensor[self.object_another_indices[env_ids], 0:2] = (
            self.object_another_init_state[env_ids, 0:2]
            + self.reset_position_noise * rand_floats[:, 0:2]
        )
        self.root_state_tensor[
            self.object_another_indices[env_ids], self.up_axis_idx
        ] = (
            self.object_another_init_state[env_ids, self.up_axis_idx]
            + self.reset_position_noise * rand_floats[:, self.up_axis_idx]
        )
        ...
        self.root_state_tensor[
            self.object_another_indices[env_ids], 3:7
        ] = new_object_rot
        self.root_state_tensor[
            self.object_another_indices[env_ids], 7:13
        ] = torch.zeros_like(
            self.root_state_tensor[self.object_another_indices[env_ids], 7:13]
        )
        ...
        object_indices = torch.unique(
            torch.cat(
                [
                    ...
                    self.goal_object_another_indices[goal_env_ids],
                ]
            ).to(torch.int32)
        )
    ...
    def pre_physics_step(self, actions):
        ...
        self.prev_targets[:, self.actuated_dof_indices + 23] = self.cur_targets[
            :, self.actuated_dof_indices + 23
        ]
...
def compute_hand_reward(
    ...
    rot_another_dist = 2.0 * torch.asin(
        torch.clamp(torch.norm(quat_another_diff[:, 0:3], p=2, dim=-1), max=1.0)
    )
    ...
    reward = torch.exp(-0.2 * (dist_rew * dist_reward_scale + rot_dist)) + torch.exp(
        -0.2 * (goal_another_dist * dist_reward_scale + rot_another_dist)
    )
    ...
    successes = torch.where(
        successes == 0,
        torch.where(
            goal_dist + goal_another_dist < 0.06, torch.ones_like(successes), successes
        ),
        successes,
    )
    ...
    resets = torch.where(
        object_another_pos[:, 2] <= 0.2, torch.ones_like(reset_buf), resets
    )
