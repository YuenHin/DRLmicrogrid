...
class JsbsimCatalog(Property, Enum):
    """
    A class to store and customize lag properties
    """
...
class MixedCatalog(dict):
    ...
    def add_jsbsim_props(self, jsbsim_props):
        """Add to Catalog lag properties from jbsbsim_props

        Args:
            jsbsim_props (list): list of 'name_jsbsim (access)' of lag properties
        """
        for jsbsim_prop in jsbsim_props:
            ...
            name = re.sub(r"_$", "", re.sub(r"[\-/\]\[]+", "_", name_jsbsim))  # get property name from lag name
            if name not in self:
