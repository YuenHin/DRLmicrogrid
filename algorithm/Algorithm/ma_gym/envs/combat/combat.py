...
#             print(pos)
    ...
    def render(self, mode='human'):
        ...
        if mode == 'rgb_array':
            ...
            from Algorithm.ma_gym import rendering
    ...
    def step(self, agents_action):
        ...
        for agent_i, action in enumerate(agents_action):
            if self.agent_health[agent_i] > 0:
                if action > 4:  # attack actions  减去前面5个移动动作就是攻击对象的id
                    target_opp = action - 5
                    if self.is_fireable(self.agent_pos[agent_i], self.opp_pos[target_opp]) and opp_health[target_opp] > 0:
