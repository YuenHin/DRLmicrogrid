...
class test_DDPG():
    def func(self, train_time, sigma):
        ...
        # plt.title('DDPG on {}'.format(title[0]))
        plt.title('DDPG on UIES return')
        plt.savefig('.\Data\RL_RO\DDPG on UIES return' + '_1000.png')
        # plt.show()
        ...
        plt.savefig('.\Data\RL_RO\DDPG on UIES Punishment' + str(train_time) + '_1000.png')
        # plt.show()
        ###############################################################################
        # 保存到excel
        # writeDatatoExcel(".\Data\RL_RO\Result\DDPG_actor_testv6.0.xlsx", 1, 0, return_list[0])
        # writeDatatoExcel(".\Data\RL_RO\Result\DDPG_actor_testv6.0.xlsx", 2, 0, storage_punishment_list)

        # 保存模型参数
        torch.save(agent.actor.state_dict(), '.\Data\RL_RO\model\DDPG_actor_network'+ '_' + '_1000.pkl')
        torch.save(agent.critic.state_dict(), '.\Data\RL_RO\model\DDPG_critic_network'+ '_' + '_1000.pkl')
